...
# if 201103L <= YY_CPLUSPLUS
  ...
  private:
    ...
    union union_type
    {
      ...
      // struct_val
      // struct_list
      char dummy6[sizeof (std::map<std::string, ptc::ir::Expr *> )];
      ...
      // matvals
      // matsq
      char dummy8[sizeof (std::vector<ptc::ir::Expr *> )];

      // funargsnvar
      // funargs
      char dummy9[sizeof (std::vector<ptc::ir::FormalParamDecl *> )];
    ...
    struct symbol_kind
    {
      enum symbol_kind_type
      {
        ...
        S_KWVAR = 57,                            // "var"
        S_KWANY = 58,                            // "any"
        S_KWIMPORT = 59,                         // "import"
        S_KWIF = 60,                             // "if"
        S_KWELSE = 61,                           // "else"
        S_KWFOR = 62,                            // "for"
        S_KWWHILE = 63,                          // "while"
        S_KWDO = 64,                             // "do"
        S_KWRETURN = 65,                         // "return"
        S_KWBREAK = 66,                          // "break"
        S_KWCONTINUE = 67,                       // "continue"
        S_KWCONST = 68,                          // "const"
        S_KWMAYBE = 69,                          // "?"
        S_KWINT = 70,                            // "'int'"
        S_KWFLOAT = 71,                          // "'float'"
        S_KWSTRING = 72,                         // "'string'"
        S_KWBOOL = 73,                           // "'bool'"
        S_KWSTRUCT = 74,                         // "'struct'"
        S_KWVOID = 75,                           // "'void'"
        S_KWRANGE = 76,                          // "'range'"
        S_ID = 77,                               // "identifier"
        S_EXT_ID = 78,                           // "external identifier"
        S_NEG = 79,                              // NEG
        S_YYACCEPT = 80,                         // $accept
        S_start = 81,                            // start
        S_stmt = 82,                             // stmt
        S_stmt_end = 83,                         // stmt_end
        S_stmts = 84,                            // stmts
        S_stmts_ne = 85,                         // stmts_ne
        S_block = 86,                            // block
        ...
        S_el_kw = 101,                           // el_kw
        S_struct = 102,                          // struct
        S_decllist = 103,                        // decllist
        S_declistval = 104,                      // declistval
        S_function = 105,                        // function
        S_fun_id = 106,                          // fun_id
        S_funargsnvar = 107,                     // funargsnvar
        S_funargs = 108,                         // funargs
        S_vardecl = 109,                         // vardecl
        S_vardef = 110,                          // vardef
        S_set = 111,                             // set
        S_callarglist = 112,                     // callarglist
        S_expr = 113,                            // expr
        S_expr_var = 114,                        // expr_var
        S_matrix = 115,                          // matrix
        S_matvals = 116,                         // matvals
        S_expr_mat = 117,                        // expr_mat
        S_range = 118,                           // range
        S_int_val = 119,                         // int_val
        S_slice = 120,                           // slice
        S_expr_none = 121,                       // expr_none
        S_expr_struct = 122,                     // expr_struct
        S_struct_val = 123,                      // struct_val
        S_struct_list = 124,                     // struct_list
        S_expr_int = 125,                        // expr_int
        S_expr_float = 126,                      // expr_float
        S_expr_str = 127,                        // expr_str
        S_expr_bool = 128,                       // expr_bool
        S_funtype = 129,                         // funtype
        S_typelist = 130,                        // typelist
        S_mattype = 131,                         // mattype
        S_matsq = 132,                           // matsq
        S_type = 133                             // type
      };
    };
        ...
        switch (this->kind ())
    {
      ...
      case symbol_kind::S_struct_list: // struct_list
        value.move< std::map<std::string, ptc::ir::Expr *>  > (std::move (that.value));
        break;
...
#if 201103L <= YY_CPLUSPLUS
      basic_symbol (typename Base::kind_type t, std::map<std::string, ptc::ir::Expr *> && v, location_type&& l)
        : Base (t)
        , value (std::move (v))
        , location (std::move (l))
      {}
#else
      basic_symbol (typename Base::kind_type t, const std::map<std::string, ptc::ir::Expr *> & v, const location_type& l)
        : Base (t)
        , value (v)
        , location (l)
      {}
...
switch (yykind)
    {
      ...
      case symbol_kind::S_struct_val: // struct_val
      case symbol_kind::S_struct_list: // struct_list
        value.template destroy< std::map<std::string, ptc::ir::Expr *>  > ();
        break;
  ...
  private:
    ...
    enum
    {
      yylast_ = 2655,     ///< Last index in yytable_.
